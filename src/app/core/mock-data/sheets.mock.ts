...
export const EVALUATORS = ['Claire Berthier', 'Yanis Morel', 'Sophie Lemaire', 'Ibrahim Traoré', 'Marc Dupont'] as const;
...
export function sheetsFor(student: StudentDirectoryItem): PedagogicalSheet[] {
  ...
  return SHEET_TITLES.map((title, index) => {
    ...
    else if (number <= Math.max(student.validatedSheets + rework, student.presentedSheets)) status = 'presented';
    else if (number <= student.preparedSheets) status = number % 2 === 0 ? 'ready' : 'in_progress';

    const isPresented = status === 'presented' || status === 'validated' || status === 'rework';
    return {
      number,
      titleKey: `sheets.titles.${title}`,
      status,
      preparationDate: status === 'not_started' ? undefined : `2026-0${(number % 9) + 1}-1${number % 9}`,
      presentationDate: isPresented ? `2026-0${(number % 9) + 1}-2${number % 8}` : undefined,
      durationMinutes: isPresented ? 45 : undefined,
      evaluator: isPresented ? EVALUATORS[number % EVALUATORS.length] : undefined,
      commentKey: status === 'rework' ? 'sheets.comments.rework' : status === 'validated' ? 'sheets.comments.validated' : undefined,
    };
  });
}
