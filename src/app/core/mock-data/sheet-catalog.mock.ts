...
export type SheetCategory = 'rules' | 'risk' | 'vehicle' | 'pedagogy' | 'exam' | 'mobility';
...
export const DEFAULT_SHEET_CATALOG: SheetCatalogItem[] = SHEET_TITLES.map((title, index) => ({
  id: `sheet-${index + 1}`,
  number: index + 1,
  ...
  durationMinutes: 40,
  active: true,
  reference: index < 40 ? 'Code de la route / référentiel ECSR' : 'Référentiel TP ECSR / REMC',
}));
