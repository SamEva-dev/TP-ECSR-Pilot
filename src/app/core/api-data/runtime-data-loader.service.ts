...
export class RuntimeDataLoaderService {
  ...
  async hydrateWorkspace(
    ...
    replaceRuntimeArray(
      ORGANIZATIONS,
      dto.organizations.map((x) => ({ ...x, id: x.key, apiId: x.id })),
    );
    replaceRuntimeArray(
      TRAINING_SITES,
      dto.sites.map((x) => ({
        ...
        id: x.key,
        apiId: x.id,
        organizationId: x.organizationKey,
      })),
    );
    replaceRuntimeArray(
      TRAINING_PROGRAMS,
    ...
    replaceRuntimeArray(
      ...
      dto.cohorts.map((x) => ({
        ...
        apiId: x.id,
        offeringId:
          dto.offerings.find((o) => o.id === x.programOfferingId)?.key ??
          x.programOfferingId,
        shortName: x.code,
        start: x.startDate,
        end: x.endDate,
        studentCount: x.learnerCount,
        capacity: x.capacity,
      })),
    );
    replaceRuntimeArray(
      CONTEXTUAL_PROMOTIONS,
      WORKSPACE_COHORTS.map((c) => ({
        ...c,
        programId: dto.offerings.find((o) => o.key === c.offeringId)
          ?.programKey,
        siteId: dto.offerings.find((o) => o.key === c.offeringId)?.siteKey,
      })),
    );

    const programId =
      selection?.programId ?? dto.defaultSelection?.programId ?? undefined;
    const cohortId =
      selection?.cohortId ?? dto.defaultSelection?.cohortId ?? undefined;
    const siteId =
      selection?.siteId ?? dto.defaultSelection?.siteId ?? undefined;
    const organizationId =
      selection?.organizationId ??
      dto.defaultSelection?.organizationId ??
      undefined;
    await this.hydrateContext(
      { organizationId, siteId, programId, cohortId },
      dto,
      isCurrent,
    );
  }
  ...
  async hydrateContext(
    ...
    const cohort =
      (dto?.cohorts ?? []).find(
        (x) => x.id === selection.cohortId || x.key === selection.cohortId,
      ) ??
      WORKSPACE_COHORTS.find(
        (x) => x.apiId === selection.cohortId || x.id === selection.cohortId,
      );
    const cohortApiId = cohort?.apiId ?? cohort?.id;
    const referentialVersionId = cohort?.referentialVersionId;
    const organizationApiId =
      (dto?.organizations ?? []).find(
        (x) =>
          x.id === selection.organizationId ||
          x.key === selection.organizationId,
      )?.id ??
    ...
    replaceRuntimeArray(
      TRAINING_REFERENTIALS,
      (data.referentials ?? []).map((x: any) => ({
        ...x,
        id: x.key ?? x.id,
        apiId: x.id,
        programId: x.programKey ?? x.programId,
        version: x.version,
    ...
    const learners = (data.learners ?? []).map((x: any) => ({
      id: x.learnerProfileId,
      apiId: x.learnerProfileId,
      enrollmentId: x.enrollmentId,
      firstName: x.firstName,
      lastName: x.lastName,
      email: x.email,
      phone: x.phone,
      promotionId: cohort?.key ?? cohort?.id ?? "",
      promotionName: cohort?.name ?? "",
      enrollmentStatus: x.enrollmentStatus?.toLowerCase(),
      progress: 0,
      completedHours: 0,
      catchupHours: 0,
      preparedSheets: 0,
    ...
    const sessions = (data.sessions ?? []).map((x: any) => {
      const start = new Date(x.startsAtUtc);
      const end = new Date(x.endsAtUtc);
      return {
        id: x.id,
        apiId: x.id,
        promotionId: cohort?.key ?? cohort?.id ?? "",
        promotion: cohort?.name ?? "",
        type: this.sessionType(x.type),
        titleKey: "",
        title: x.title,
        date: start.toISOString().slice(0, 10),
        start: start.toISOString(),
        end: end.toISOString(),
        time: `${this.hhmm(start)}–${this.hhmm(end)}`,
        trainer: x.trainerDisplayName ?? "",
        meta: x.location ?? "",
        location: x.location ?? "",
        expected: x.expectedLearners ?? 0,
        present: x.presentLearners ?? 0,
        objectiveKey: "",
        supportsKey: "",
        modality: x.modality,
        status: x.status,
    ...
    replaceRuntimeArray(
      PLANNING_EVENTS,
      sessions.map((x: any) => ({ ...x, title: x.title, meta: x.meta })),
    );
    replaceRuntimeArray(
      DRIVING_PROGRAMMED,
      sessions.filter((x: any) => x.type === "driving"),
    );
    replaceRuntimeArray(
      SESSION_TRAINERS,
      [...new Set(sessions.map((x: any) => x.trainer).filter(Boolean))].map(
        (name: any, i) => ({ id: `trainer-${i}`, name, label: name }),
      ),
    );
    replaceRuntimeArray(PEDAGOGICAL_TEAM, SESSION_TRAINERS);

    const periods = (data.workplace ?? []).map((x: any) => ({
      id: x.id,
      studentId: x.learnerExternalKey ?? x.enrollmentId,
      studentName: x.learnerDisplayName,
      company: x.company,
      city: x.city,
      tutor: x.tutorName,
      ...
      status: x.status,
      trainerVisible: x.trainerVisible,
      activities: x.activities ?? [],
      documents: x.documents ?? [],
      tutorObservationKey: x.tutorObservation ?? "",
    }));
    ...
    replaceRuntimeArray(
      ...
      candidates.map((x: any) => ({
        ...x,
        studentId: x.enrollmentId,
        firstName: x.firstName ?? x.learnerDisplayName?.split(" ")[0] ?? "",
        lastName:
          x.lastName ??
          x.learnerDisplayName?.split(" ").slice(1).join(" ") ??
          "",
        ready: x.eligible,
        unitStatuses: x.unitStatuses ?? [],
        steps: x.assessments ?? [],
      })),
    );
    (globalThis as any).__pedagoraCertificationSchemes = data.certSchemes ?? [];

    const topics = (data.topics ?? []).map((x: any) => ({
      id: x.id,
      apiId: x.id,
      number: x.number,
      code: x.code,
    ...
    const comps = (data.competencies ?? []).map((x: any) => ({
      id: x.id,
      apiId: x.id,
      code: x.code,
      title: x.title,
    ...
    replaceRuntimeArray(
      ...
      comps.filter(
        (x: any) =>
          x.kind?.toLowerCase().includes("driv") ||
          x.code?.toUpperCase().startsWith("C"),
      ),
    );
    replaceRuntimeArray(
      DRIVING_SUB_SKILLS,
      comps.filter((x: any) => x.parentId),
    );
    ...
    replaceRuntimeArray(
      REMOTE_WORK_ACTIVITIES,
      (data.remoteWork ?? []).flatMap((x: any) => x.activities ?? []),
    );
    ...
    replaceRuntimeArray(
      ORGANIZATION_ALERTS,
      orgDash?.openAlerts
        ? [{ id: "open-alerts", count: orgDash.openAlerts }]
        : [],
    );
    replaceRuntimeArray(
      PROMOTION_METRICS,
      data.cohortDashboard ? [data.cohortDashboard] : [],
    );
    replaceRuntimeArray(
      SITE_PERFORMANCES,
      data.siteDashboard ? [data.siteDashboard] : [],
    );
    ...
    replaceRuntimeArray(CENTER_STATUSES, []);
    replaceRuntimeArray(CENTER_RANKING, []);
    replaceRuntimeArray(CENTER_COMPETENCIES, comps);
    replaceRuntimeArray(STUDENT_COMPETENCIES, comps);
    replaceRuntimeArray(TRAINER_COMPETENCIES, comps);
    replaceRuntimeArray(SECRETARIAT_PRIORITIES, []);
    replaceRuntimeArray(
      SUCCESS_ANALYTICS_RECORDS,
      this.successFromCertification(candidates, cohort, selection),
    );

    replaceRuntimeArray(DRIVING_TRAINERS, SESSION_TRAINERS);
    replaceRuntimeArray(DRIVING_HISTORY, []);
    replaceRuntimeArray(DRIVING_OBSERVATIONS, []);
    replaceRuntimeArray(DRIVING_CRITERIA, []);
    replaceRuntimeArray(
      ATTENDANCE_STUDENTS,
      learners.map((x: any) => ({
  ...
  private sessionType(value: string): string {
    const v = (value ?? "").toLowerCase();
    return v === "classroom" ? "theory" : v;
  }
  private hhmm(d: Date): string {
    return `${String(d.getHours()).padStart(2, "0")}:${String(d.getMinutes()).padStart(2, "0")}`;
  }
  private successFromCertification(
    candidates: any[],
    cohort: any,
    selection: any,
  ): any[] {
    if (!cohort) return [];
    const graduated = candidates.filter((x) =>
      String(x.decision ?? x.result)
        .toLowerCase()
        .includes("obtain"),
    ).length;
    ...
    return [
      {
        id: cohort.key ?? cohort.id,
        organizationId: selection.organizationId,
        siteId: selection.siteId,
        programId: selection.programId,
        cohortId: cohort.key ?? cohort.id,
        promotionName: cohort.name,
        ...
        failed,
        absent: 0,
        rate: candidates.length
          ? Math.round((graduated * 100) / candidates.length)
          : 0,
        candidates,
      },
    ];
