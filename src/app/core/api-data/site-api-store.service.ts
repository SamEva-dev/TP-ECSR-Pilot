...
export class SiteApiStoreService {
  ...
  private readonly sitesSignal = signal<SiteProfile[]>([]);
      ...
      void firstValueFrom(
        this.http.post(
          `${environment.apiBaseUrl}/api/v1/organizations/${org.apiId}/sites`,
          {
            ...
            name: value.name,
            city: value.city,
            externalKey: `site-${value.code.toLowerCase()}`,
          },
        ),
      ).then(() => this.workspace.reload());
    return {
      id: "pending",
      organizationId,
      ...value,
  ...
  update(id: string, value: SiteFormValue): void {
    const site = this.workspace.sites().find((x) => x.id === id);
    const org = this.workspace
      .organizations()
      .find((x) => x.id === site?.organizationId);
    if (site?.apiId && org?.apiId)
      void firstValueFrom(
        this.http.put(
          `${environment.apiBaseUrl}/api/v1/organizations/${org.apiId}/sites/${site.apiId}`,
          { name: value.name, city: value.city, status: value.status },
        ),
      ).then(() => this.workspace.reload());
  }
  byId(id: string): SiteProfile | undefined {
    this.sync();
    return this.sitesSignal().find((x) => x.id === id);
  }
}
