...
export class OrganizationDashboardComponent {
  ...
  readonly organizationId = computed(() => this.workspace.organization()?.id ?? "org-aftral");
  readonly kpis = computed(
    () => ORGANIZATION_KPIS.find((item) => item.organizationId === this.organizationId()) ?? ORGANIZATION_KPIS[0],
  );
  readonly sites = computed(() => SITE_PERFORMANCES.filter((item) => item.organizationId === this.organizationId()));
  readonly programs = computed(() => PROGRAM_PERFORMANCES.filter((item) => item.organizationId === this.organizationId()));
  readonly alerts = computed(() => ORGANIZATION_ALERTS.filter((item) => item.organizationId === this.organizationId()));
  readonly activity = computed(() => ORGANIZATION_ACTIVITY.filter((item) => item.organizationId === this.organizationId()));
  ...
  openProgram(programId: string): void {
    const accessibleSite = this.workspace.sites().find((site) =>
      PROGRAM_OFFERINGS.some(
        (offering) => offering.siteId === site.id && offering.programId === programId && offering.active,
      ),
    );
