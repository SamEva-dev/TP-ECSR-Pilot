...
export class ProgramDetailComponent {
  ...
  readonly activeReferential = computed(
    () =>
      TRAINING_REFERENTIALS.find(
        (item) => item.programId === this.programId && item.status === "active",
      ) ?? null,
  );
  ...
  readonly cohorts = computed(() => {
    ...
    return WORKSPACE_COHORTS.flatMap((cohort) => {
      const offering = PROGRAM_OFFERINGS.find(
        (item) =>
          item.id === cohort.offeringId && item.programId === program.id,
      );
      if (!offering) return [];
      const site = this.workspace
        .sites()
        .find((item) => item.id === offering.siteId);
      if (!site || site.organizationId !== this.workspace.organization()?.id)
        return [];
  ...
  toggleSite(siteId: string): void {
    this.store.toggleSite(this.programId, siteId);
  }
