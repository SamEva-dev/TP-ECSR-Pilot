...
export class HomeComponent {
  ...
  readonly promotion = computed(() =>
    this.workspace.cohort()
    ?? PROMOTIONS.find((p) => p.id === this.sessionService.promotionId())
    ?? PROMOTIONS[0]
    ?? null,
  );

  readonly promotionStart = computed(() => this.formatDate(this.promotion()?.start));
  readonly promotionEnd = computed(() => this.formatDate(this.promotion()?.end));
