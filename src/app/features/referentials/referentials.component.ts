...
export class ReferentialsComponent {
  ...
  readonly rows = computed(() => {
    ...
    return this.store
      ...
      .map((referential) => ({
        ...referential,
        program: PROGRAM_CATALOG.find(
          (program) => program.id === referential.programId,
        ),
      }))
      .filter((item) => {
        const matchesQuery =
          !query ||
          `${item.name} ${item.code} ${item.version}`
            .toLowerCase()
            .includes(query);
        const matchesProgram =
          this.programId() === "all" || item.programId === this.programId();
        const matchesStatus =
  ...
  readonly activeCount = computed(
    () => this.store.items().filter((item) => item.status === "active").length,
  );
  readonly draftCount = computed(
    () => this.store.items().filter((item) => item.status === "draft").length,
  );
  readonly archivedCount = computed(
    () =>
      this.store.items().filter((item) => item.status === "archived").length,
  );
  ...
  setProgram(event: Event): void {
    this.programId.set((event.target as HTMLSelectElement).value);
  }
  setStatus(event: Event): void {
    this.status.set(
      (event.target as HTMLSelectElement).value as ReferentialStatus | "all",
    );
  }
