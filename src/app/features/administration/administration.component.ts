...
} from "../../core/models/organization-administration.models";
...
export class AdministrationComponent {
  ...
  readonly profile = signal<OrganizationAdminProfile>(
    organizationProfileFor(""),
  );
  readonly branding = signal<OrganizationBrandingSettings>(
    organizationBrandingFor(""),
  );
  readonly modules = signal(
    ORGANIZATION_MODULE_SETTINGS.map((item) => ({ ...item })),
  );
  readonly preferences = signal(
    ORGANIZATION_PREFERENCES.map((item) => ({ ...item })),
  );
  readonly audit = ORGANIZATION_AUDIT;
  readonly saved = signal(false);
  readonly brandingSaved = signal(false);
  readonly activeSettingsTab = signal<"general" | "notifications" | "security">(
    "general",
  );
  ...
  readonly programIds = computed(() => {
    ...
    return [
      ...new Set(
        PROGRAM_OFFERINGS.filter(
          (offering) => siteIds.includes(offering.siteId) && offering.active,
  ...
  readonly programs = computed(() =>
    TRAINING_PROGRAMS.filter((program) =>
      this.programIds().includes(program.id),
    ),
  );

  readonly cohorts = computed(() => {
    const siteIds = this.sites().map((site) => site.id);
    const offeringIds = PROGRAM_OFFERINGS.filter((offering) =>
      siteIds.includes(offering.siteId),
  ...
  readonly metrics = computed(() =>
    organizationMetricsFor(this.workspace.organization()?.id),
  );
  readonly activeCohorts = computed(
    () => this.cohorts().filter((cohort) => cohort.status === "active").length,
  );

  readonly academicYear = computed(() => {
    const cohort = this.workspace.cohort();
    if (!cohort?.start || !cohort?.end) return "—";
    return `${new Date(cohort.start).getFullYear()}–${new Date(cohort.end).getFullYear()}`;
  });
  ...
  readonly managementCards = [
    {
      ...
      titleKey: "organizationAdmin.cards.sites.title",
      descriptionKey: "organizationAdmin.cards.sites.description",
      tone: "blue",
    },
    {
      path: "/formations",
      icon: "ph-books",
      titleKey: "organizationAdmin.cards.programs.title",
      descriptionKey: "organizationAdmin.cards.programs.description",
      tone: "orange",
    },
    {
      path: "/referentiels",
      icon: "ph-stack",
      titleKey: "organizationAdmin.cards.referentials.title",
      descriptionKey: "organizationAdmin.cards.referentials.description",
      tone: "green",
    },
    {
      path: "/acces",
      icon: "ph-shield-check",
      titleKey: "organizationAdmin.cards.access.title",
      descriptionKey: "organizationAdmin.cards.access.description",
      tone: "purple",
    },
    {
      path: "/teletravail",
      icon: "ph-house-line",
      titleKey: "organizationAdmin.cards.remoteWork.title",
      descriptionKey: "organizationAdmin.cards.remoteWork.description",
      tone: "blue",
    },
  ] as const;
  ...
  updateBranding(
    field: "primaryColor" | "secondaryColor" | "loginTagline" | "logoLabel",
    value: string,
  ): void {
  ...
  toggleRemoteWorkPolicy(
    field: "enabled" | "approvalRequired" | "halfDayAllowed" | "endOfDayReport",
  ): void {
    this.remoteWorkPolicy.update((current) => ({
      ...current,
      [field]: !current[field],
    }));
  }
